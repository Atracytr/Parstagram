...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<Button>(R.id.btSubmit).setOnClickListener() {
            ...
            val description = findViewById<EditText>(R.id.etDiscription).text.toString()
            val user = ParseUser.getCurrentUser()
            if (photoFile != null) {
                submitPost(description, user, photoFile!!)
            } else {
                Log.e(TAG, "Error in submitting picture")
    ...
    fun submitPost(description: String, user: ParseUser, file: File) {
        ...
        post.saveInBackground { exception ->
            if (exception != null) {
                ...
                Toast.makeText(this, "Something went wrong with saving post.", Toast.LENGTH_SHORT). show()
            } else
